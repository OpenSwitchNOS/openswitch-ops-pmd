...
VLOG_DEFINE_THIS_MODULE(pm_plugins);
...
struct plugin_class {
    ...
    const struct pm_subsystem_class_t *(*subsystem_class_get)(void);
    const struct pm_module_class_t *(*module_class_get)(void);
};
...
plugin_open(const char *filename, void *data)
{
    ...
    rc = !(pc->plugin_init = lt_dlsym(dlhandle, "pm_plugin_init"));
    CHECK_LT_RC(rc, "Load pm_plugin_init");

    rc = !(pc->plugin_deinit = lt_dlsym(dlhandle, "pm_plugin_deinit"));
    CHECK_LT_RC(rc, "Load pm_plugin_deinit");

    rc = !(pc->plugin_run = lt_dlsym(dlhandle, "pm_plugin_run"));
    CHECK_LT_RC(rc, "Load pm_plugin_run");

    rc = !(pc->plugin_wait = lt_dlsym(dlhandle, "pm_plugin_wait"));
    CHECK_LT_RC(rc, "Load pm_plugin_wait");

    rc = !(pc->subsystem_class_get = lt_dlsym(dlhandle, "pm_subsystem_class_get"));
    CHECK_LT_RC(rc, "Load pm_subsystem_class_get");

    rc = !(pc->module_class_get = lt_dlsym(dlhandle, "pm_module_class_get"));
    CHECK_LT_RC(rc, "Load pm_module_class_get");
...
pm_plugins_load(void)
{
    ...
    rc = !(interface_id = lt_dlinterface_register("pm-plugin", NULL));
...
pm_subsystem_class_get(const char *platform_type)
{
    lt_dlhandle iter_handle = 0;
    struct plugin_class *pc = NULL;
...
pm_module_class_get(const char *platform_type)
{
    lt_dlhandle iter_handle = 0;
    struct plugin_class *pc = NULL;
